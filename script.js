...
const state = {
  ...
  apiKey: localStorage.getItem('bf_gemini_key') || '',
  view:'dashboard'
};
...
async function askGemini(prompt){if(!state.apiKey) throw new Error('Add your Gemini API key in the AI advisor panel first.');const t=totals();const snapshot={month:currentMonth(),income:t.income,spent:t.spent,saved:t.saved,savingsRate:`${t.rate.toFixed(1)}%`,transactions:state.transactions.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,40),budgets:state.budgets,goals:state.goals};const system=`You are BudgetFlow AI, a careful personal budgeting assistant. Give practical, conservative budgeting guidance based only on the user's supplied data. Do not claim to be a financial adviser. Never promise investment returns. Use GBP (£) and UK spelling. Flag missing information. Keep the answer clear with short sections and actionable steps. Here is the user's current local budget snapshot:\n${JSON.stringify(snapshot,null,2)}\n\nUser question: ${prompt}`;const res=await fetch(API_URL,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':state.apiKey},body:JSON.stringify({contents:[{role:'user',parts:[{text:system}]}],generationConfig:{temperature:0.4,maxOutputTokens:900}})});if(!res.ok){let msg='Gemini request failed';try{const e=await res.json();msg=e?.error?.message||msg}catch{}throw new Error(msg)}const data=await res.json();return data?.candidates?.[0]?.content?.parts?.map(p=>p.text||'').join('')||'No response received.'}
